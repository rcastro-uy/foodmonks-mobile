...
export default function HighScore({toastRef, idPedido, setMostrarModal,setExisteCalificacion, setRefrescar } : any) {
    ...
    const onSubmit = async () => {
        ...
        const resp = await altaCalificacion(idPedido,calificacion.toString(),text)
        setLoading(false)
    ...
    return (
        ...
        <View style={{margin:5}}>
            <Text style={{fontSize:25, fontWeight:'bold', alignSelf:'center',bottom:10}}>¿Como estuvo el servicio? </Text>
            <AirbnbRating
                count={5}
                reviews={["Muy malo", "Malo", "Bueno", "Muy bueno", "Excelente"]}
                defaultRating={calificacion}
                onFinishRating= {(number) => setCalificacion(number)}
                size={50}
            />
             <TextInput
                mode="outlined"
